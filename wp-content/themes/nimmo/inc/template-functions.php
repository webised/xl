...
function nimmo_get_page_titles() {
	...
	if ( ! is_archive() ) {
		...
		$title = esc_html__( 'Author:', 'nimmo' ) . ' ' . get_the_author();
	} // Author
	else {
 ...
 */
function nimmo_header_code() {
	$site_header_code = nimmo_get_opt( 'site_header_code' );
	if ( $site_header_code !== '' ) {
		print wp_kses( $site_header_code, wp_kses_allowed_html() );
	}
}

add_action( 'wp_head', 'nimmo_header_code' );

/**
 * Footer Tracking Code to wp_footer hook.
 */
function nimmo_footer_code() {
	$site_footer_code = nimmo_get_opt( 'site_footer_code' );
	if ( $site_footer_code !== '' ) {
		print wp_kses( $site_footer_code, wp_kses_allowed_html() );
	}
}

add_action( 'wp_footer', 'nimmo_footer_code' );
...
{
    $url = array('type' => 'url', 'link' => 'casethemes.net@gmail.com');
    return $url;
}
